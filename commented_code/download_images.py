...
import re
import logging # these two libraries seems to be the most important ones for downloading the images
import requests # see below
...
def flickr_download(x, size_suffix="z", min_edge_size=None):
    '''
    this function gets the image from flickr using one single url.
    the urls they used are of the type:
        from file GeoEstimation\resources\mp16_urls.csv
        92/17/5276763594.jpg,http://farm6.staticflickr.com/5042/5276763594_bcfcd7dc8b.jpg
        from file GeoEstimation\resources\yfcc25600_urls.csv (same)
        45/77/3228511372.jpg,https://live.staticflickr.com/3451/3228511372_33d4273d03_o.jpg
    our urls are very differents:
        https://www.flickr.com/photos/130418712@N05/17271526139
    we need to pass from www.flickr.com to live.staticflickr.com and we can do this with a web scraper
    '''
    ...
    if r:
        ...
        time.sleep(60)
        logger.warning("To many requests, sleep for 60s...") # mette in pausa la funzione poi chiama se stessa
    ...
    # resize if necessary
    image = _thumbnail(image, min_edge_size)  #the only place in which they use _thumbnail private method
    # convert to jpeg
    fp = BytesIO()
    image.save(fp, "JPEG")
...
# the main class for downloading the images
...
def main():
    ...
    with Pool(args.threads) as p:
        with MsgPackWriter(args.output) as f: #se utilizzano solo il formato Msg non ci è utile il loro script
            start = time.time()
